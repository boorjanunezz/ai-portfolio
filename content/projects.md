...
<!--
  ...
  Campos reconocidos por la web:
    ...
    EN:          resumen en inglés para la web en inglés (opcional)
...
EN: Master's thesis built for Ricoh Spain: a RAG chatbot to query EU regulation (GDPR, AI Act, NIS2) with rigorous source citation, on a Medallion architecture.
...
EN: ETL pipeline that digitises invoices: extracts key fields from unstructured PDFs with Azure Document Intelligence and loads them into SQL Server.
...
EN: This site: an AI assistant over a lightweight, database-free RAG (BM25 over Markdown) that cites its sources.
...
EN: Emotional-support web app: empathetic chat with Qwen 2.5 72B, crisis detection, guided breathing and a mood log.
...
EN: LangGraph agent that analyses news using Azure OpenAI models deployed on Azure AI Foundry.
GitHub:
Demo:
Imagen:
...
EN: Telegram bot orchestrated with n8n: understands voice notes with Whisper, replies with GPT-4o-mini and TTS-HD.
GitHub:
Demo:
Imagen:
...
EN: Hands-on work with Azure AI Foundry: guardrails, content filters, reasoning and multimodal models.
...
EN: Streamlit app serving a handwritten-digit CNN and a house-price predictor.
GitHub:
Demo:
Imagen:
...
EN: Master's generative-AI module: prompt engineering, fine-tuning, embeddings, RAG and neural networks.
...
EN: Browser geography game on an interactive SVG world map: 175 countries, 6 game modes, no framework or backend.
